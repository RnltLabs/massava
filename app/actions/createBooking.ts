...
export async function createBooking(
  ...
  try {
    ...
    // For just-registered users: registeredUserId comes from server-side signUp() result
    // This is safe because it was generated by our signUp action, not arbitrary client input
    const effectiveCustomerId = authenticatedUserId || validated.registeredUserId || undefined

    // Prepare booking data
    const bookingData = {
      studioId: validated.studioId,
      serviceId: validated.serviceId,
      customerId: effectiveCustomerId, // ✅ SECURITY: Session user > just-registered user > guest
