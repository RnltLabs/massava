...
import { addressSchema } from '@/app/[locale]/dashboard/_components/studio-registration/validation/studioSchemas';
import { cn } from '@/lib/utils';
import { updateStudioAddress } from '@/app/[locale]/business/actions/profile';
...
export function AddressPopup({ open, onOpenChange, initialData }: AddressPopupProps): React.JSX.Element {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      const validatedAddress = addressSchema.parse({
        ...
        city,
        postalCode,
        country: 'Deutschland', // Default country - not stored in DB
      });
  ...
  const content = (
    <>
      ...
      <div className="space-y-4">
        ...
        {/* Address Preview - Paket-Format */}
        {(street || city || postalCode) && (
          <div className="p-4 bg-white rounded-lg border border-gray-200">
            <p className="text-xs font-semibold text-gray-600 mb-2">Adress-Vorschau:</p>
            <div className="space-y-0.5 text-sm text-gray-900 font-mono">
              {street && <div>{street}{line2 ? `, ${line2}` : ''}</div>}
  ...
  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          <VisuallyHidden>
            <SheetTitle>Standort bearbeiten</SheetTitle>
          </VisuallyHidden>
          {content}
        </SheetContent>
      </Sheet>
