...
exports.executeUserRetention = executeUserRetention;
exports.executeHealthDataRetention = executeHealthDataRetention;
exports.executeBookingsRetention = executeBookingsRetention;
exports.executeAuditLogsRetention = executeAuditLogsRetention;
exports.executePermanentDeletion = executePermanentDeletion;
exports.executeAllRetentionPolicies = executeAllRetentionPolicies;
exports.getUsersNeedingWarnings = getUsersNeedingWarnings;
 ...
 */
function shouldDelete(data, dataType) {
    const now = new Date();
    const retentionPeriod = exports.RETENTION_PERIODS[dataType];
    // If data has deletedAt timestamp, check grace period
    if (data.deletedAt) {
        const deletionTime = new Date(data.deletedAt).getTime();
        const gracePeriod = exports.RETENTION_PERIODS.SOFT_DELETE_GRACE;
 ...
 */
function shouldWarn(data, dataType) {
    ...
    const now = new Date();
    const retentionPeriod = exports.RETENTION_PERIODS[dataType];
    const warningPeriod = exports.RETENTION_PERIODS.WARNING_PERIOD;
    const lastActivity = data.updatedAt || data.createdAt;
    const timeSinceLastActivity = now.getTime() - new Date(lastActivity).getTime();
 ...
 */
async function executeUserRetention(dryRun = false) {
    const errors = [];
    let deletedCount = 0;
    let warnings = 0;
    try {
        const now = new Date();
        const retentionThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.USER_ACCOUNT);
        ...
        logger_1.logger.info('Found inactive users for retention processing', {
        ...
        for (const user of inactiveUsers) {
            try {
                ...
                logger_1.logger.error(errorMsg, { userId: user.id, action: 'USER_RETENTION' });
                errors.push(errorMsg);
            }
        }
        // Find users approaching deletion (for warnings)
        const warningThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.USER_ACCOUNT + exports.RETENTION_PERIODS.WARNING_PERIOD);
        const usersNeedingWarning = await prisma_1.prisma.user.findMany({
            where: {
                updatedAt: {
                    lt: warningThreshold,
        ...
        if (!dryRun && usersNeedingWarning.length > 0) {
            ...
            for (const user of usersNeedingWarning) {
                await prisma_1.prisma.user.update({
                    where: { id: user.id },
                    data: {
                        deletionScheduledAt: new Date(now.getTime() + exports.RETENTION_PERIODS.WARNING_PERIOD),
                    },
                });
            }
        }
        logger_1.logger.info('User retention execution completed', {
            deletedCount,
            warnings,
        ...
        logger_1.logger.error(errorMsg, { action: 'USER_RETENTION' });
        errors.push(errorMsg);
    }
    return {
 ...
 */
async function executeHealthDataRetention(dryRun = false) {
    const errors = [];
    let deletedCount = 0;
    try {
        const now = new Date();
        const retentionThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.HEALTH_DATA);
        // Find bookings with health data older than 1 year OR consent withdrawn
        const healthDataToDelete = await prisma_1.prisma.newBooking.findMany({
            where: {
                OR: [
                    {
        ...
        logger_1.logger.info('Found health data for retention processing', {
        ...
        for (const booking of healthDataToDelete) {
            try {
                if (!dryRun) {
                    ...
                    await prisma_1.prisma.newBooking.update({
                        where: { id: booking.id },
                        data: {
                ...
                logger_1.logger.error(errorMsg, { bookingId: booking.id, action: 'HEALTH_DATA_RETENTION' });
                errors.push(errorMsg);
            }
        }
        logger_1.logger.info('Health data retention execution completed', {
            deletedCount,
            errors: errors.length,
            dryRun,
            action: 'HEALTH_DATA_RETENTION',
        ...
        logger_1.logger.error(errorMsg, { action: 'HEALTH_DATA_RETENTION' });
        errors.push(errorMsg);
    }
    return {
 ...
 */
async function executeBookingsRetention(dryRun = false) {
    const errors = [];
    let deletedCount = 0;
    try {
        const now = new Date();
        const retentionThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.BOOKINGS);
        // Find bookings older than 3 years
        const bookingsToDelete = await prisma_1.prisma.newBooking.findMany({
            where: {
                createdAt: {
                    lt: retentionThreshold,
                },
        ...
        logger_1.logger.info('Found bookings for retention processing', {
            count: bookingsToDelete.length,
            dryRun,
            action: 'BOOKINGS_RETENTION',
        });
        if (!dryRun && bookingsToDelete.length > 0) {
            const result = await prisma_1.prisma.newBooking.deleteMany({
                where: {
                    id: {
        ...
        logger_1.logger.info('Bookings retention execution completed', {
            deletedCount,
            errors: errors.length,
            dryRun,
            action: 'BOOKINGS_RETENTION',
        ...
        logger_1.logger.error(errorMsg, { action: 'BOOKINGS_RETENTION' });
        errors.push(errorMsg);
    }
    return {
        dataType: 'BOOKINGS',
 ...
 */
async function executeAuditLogsRetention(dryRun = false) {
    const errors = [];
    let deletedCount = 0;
    try {
        const now = new Date();
        const retentionThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.AUDIT_LOGS);
        // Find audit logs older than 90 days
        const result = dryRun
            ? await prisma_1.prisma.auditLog.count({
                where: {
                    createdAt: {
                        lt: retentionThreshold,
                    },
        ...
        deletedCount = result;
        logger_1.logger.info('Audit logs retention execution completed', {
            deletedCount,
            errors: errors.length,
            dryRun,
        ...
        logger_1.logger.error(errorMsg, { action: 'AUDIT_LOGS_RETENTION' });
        errors.push(errorMsg);
    }
    return {
        dataType: 'AUDIT_LOGS',
 ...
 */
async function executePermanentDeletion(dryRun = false) {
    const errors = [];
    let deletedCount = 0;
    try {
        const now = new Date();
        const gracePeriodThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.SOFT_DELETE_GRACE);
        ...
        logger_1.logger.info('Found users for permanent deletion', {
        ...
        for (const user of usersToDelete) {
            try {
                ...
                logger_1.logger.error(errorMsg, { userId: user.id, action: 'PERMANENT_DELETION' });
                errors.push(errorMsg);
            }
        }
        logger_1.logger.info('Permanent deletion completed', {
            deletedCount,
            errors: errors.length,
            dryRun,
            action: 'PERMANENT_DELETION',
        ...
        logger_1.logger.error(errorMsg, { action: 'PERMANENT_DELETION' });
        errors.push(errorMsg);
    }
 ...
 */
async function executeAllRetentionPolicies(dryRun = false) {
 ...
 */
async function getUsersNeedingWarnings() {
    const now = new Date();
    const retentionThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.USER_ACCOUNT);
    const warningThreshold = new Date(now.getTime() - exports.RETENTION_PERIODS.USER_ACCOUNT + exports.RETENTION_PERIODS.WARNING_PERIOD);
    ...
    return users.map(user => {
        ...
        const timeUntilDeletion = exports.RETENTION_PERIODS.USER_ACCOUNT - timeSinceLastActivity;
