...
const encrypt_health_data_1 = require("@/lib/prisma/middleware/encrypt-health-data");
/**
...
function createPrismaClient() {
    ...
    if (process.env.NODE_ENV === 'production' && !isBuildPhase) {
        if (!process.env.HEALTH_DATA_ENCRYPTION_KEY) {
            throw new Error('HEALTH_DATA_ENCRYPTION_KEY must be set in production environment. ' +
                'This is required for GDPR Art. 9 compliance (health data encryption).');
        }
        return client.$extends((0, encrypt_health_data_1.createHealthDataEncryptionExtension)());
    }
    ...
    if (process.env.NODE_ENV !== 'test' && !isBuildPhase) {
        console.warn('[Prisma] Health data encryption disabled. Set HEALTH_DATA_ENCRYPTION_KEY to enable.');
    }
    return client;
}
/**
