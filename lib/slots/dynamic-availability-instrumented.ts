 ...
 * All rights reserved.
 *
 * Dynamic Availability Calculator - INSTRUMENTED VERSION
 *
 * This is a reference implementation with full GlitchTip/Sentry instrumentation.
 * Use this as a guide to add performance monitoring to dynamic-availability.ts
 * when performance monitoring becomes a priority.
 *
 * To use this version:
 * 1. Back up dynamic-availability.ts
 * 2. Copy this file to dynamic-availability.ts
 * 3. Test thoroughly
...
export async function calculateAvailableSlots(
  ...
  // Start Sentry transaction for end-to-end monitoring
  ...
  try {
    ...
    // Query 1: Fetch studio with capacity
    const studioSpan = transaction.startChild({
      op: 'db.query',
      description: 'Fetch studio details',
    });
    const studioQueryStart = performance.now();
    ...
    const studioQueryTime = performance.now() - studioQueryStart;
    studioSpan.finish();
    ...
    if (!dayHours.isOpen) {
      ...
      if (options.includeUnavailable) {
        ...
        const closedSlots = allSlots.map(time => ({
          startTime: time,
        ...
        transaction.setTag('studio_closed', 'true');
        transaction.finish();

        return ok(closedSlots);
      }
    ...
    const gridSpan = transaction.startChild({
      op: 'compute',
      description: 'Generate time grid',
    });
    const allTimeSlots = generateDayTimeSlots();
    gridSpan.finish();

    // Query 2: Fetch existing bookings for the date
    const bookingsSpan = transaction.startChild({
      op: 'db.query',
      description: 'Fetch bookings',
    });
    const bookingsQueryStart = performance.now();
    ...
    const bookingsQueryTime = performance.now() - bookingsQueryStart;
    bookingsSpan.finish();

    // Count bookings per time slot
    const mapSpan = transaction.startChild({
      op: 'compute',
      description: 'Build booking map',
    });
    ...
    // Query 3: Fetch blocked times for the date
    const blockedSpan = transaction.startChild({
      op: 'db.query',
      description: 'Fetch blocked times',
    });
    const blockedQueryStart = performance.now();
    ...
    const blockedQueryTime = performance.now() - blockedQueryStart;
    blockedSpan.finish();
    ...
    const calcSpan = transaction.startChild({
      op: 'compute',
      description: 'Calculate slot availability',
    });
    ...
    logger.info('Slot calculation completed', {
      ...
      timings: {
        ...
        studioQuery: Math.round(studioQueryTime),
        bookingsQuery: Math.round(bookingsQueryTime),
        blockedQuery: Math.round(blockedQueryTime),
        calculation: Math.round(calcTime),
      },
      counts: {
    ...
    // Add custom measurements to Sentry
    Sentry.setMeasurement('slot_calculation_total', totalTime, 'millisecond');
    Sentry.setMeasurement('slot_calculation_studio_query', studioQueryTime, 'millisecond');
    Sentry.setMeasurement('slot_calculation_bookings_query', bookingsQueryTime, 'millisecond');
    Sentry.setMeasurement('slot_calculation_blocked_query', blockedQueryTime, 'millisecond');
    Sentry.setMeasurement('slot_calculation_compute', calcTime, 'millisecond');
    Sentry.setMeasurement('booking_count', bookings.length, 'none');
    Sentry.setMeasurement('blocked_time_count', blockedTimes.length, 'none');
    Sentry.setMeasurement('total_slots', slots.length, 'none');
    Sentry.setMeasurement('available_slots', availableSlots, 'none');
    ...
    if (totalTime > 200) {
      Sentry.captureMessage('Slow slot calculation detected', {
        level: 'warning',
        tags: {
          studioId,
          date,
          performance_issue: 'slow_calculation',
        },
        extra: {
          correlationId,
          totalTime,
      ...
      logger.warn('Slow slot calculation', {
        correlationId,
        totalTime,
        threshold: 200,
        studioId,
    ...
    // Alert if booking query is slow (may need index optimization)
    if (bookingsQueryTime > 50) {
      Sentry.captureMessage('Slow booking query detected', {
        level: 'warning',
        tags: {
          studioId,
          date,
          performance_issue: 'slow_booking_query',
        },
        extra: {
          correlationId,
          bookingsQueryTime,
