 ...
 * getMinPrice([]) // 0
 */
export function getMinPrice<T extends Pick<Service, 'price'>>(
  services: T[]
): number {
 ...
 */
export function getMaxPrice<T extends Pick<Service, 'price'>>(
  services: T[]
): number {
  if (services.length === 0) {
    return 0;
 ...
 */
export function formatPriceLabel(minPrice: number): string {
  if (minPrice === 0) {
 ...
 */
export function getPriceRange<T extends Pick<Service, 'price'>>(
  services: T[]
 ...
 */
export function getAveragePrice<T extends Pick<Service, 'price'>>(
  services: T[]
): number {
  if (services.length === 0) {
    return 0;
 ...
 */
export function filterServicesByPrice<T extends Pick<Service, 'price'>>(
  services: T[],
  minPrice?: number,
  maxPrice?: number
): T[] {
